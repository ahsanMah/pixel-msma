...
def sample_many_and_save(
    model, sigmas, batch_size=1000, eps=2 * 1e-5, T=100, n_images=1, save_directory=None
):
    """
    Used for sampling big amount of images (e.g. 50000)
    :param model: model for sampling (RefineNet)
...
def sample_and_save(
    model, sigmas, x=None, eps=2 * 1e-5, T=100, n_images=1, save_directory=None
):
    """
    :param model:
    :param sigmas:
...
def main():
    ...
    samples_directory = "./samples/{}_{}_step{}/".format(
