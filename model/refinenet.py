...
from model.layers import (
    RefineBlock,
    ConditionalFullPreActivationBlock,
    ConditionalInstanceNormalizationPlusPlus2D,
    FullPreActivationBlock,
)
...
class RefineNetV1(keras.Model):
    def __init__(self, filters, activation, y_conditioned=False, splits=None):
        ...
        self.preact_1 = ConditionalFullPreActivationBlock(
            activation, filters, kernel_size=3
        )
        # FIXME: In this second preactivation block we used dilation=1, but it should have been 2
        self.preact_2 = ConditionalFullPreActivationBlock(
            activation, filters * 2, kernel_size=3, pooling=True
        )

        self.preact_3 = ConditionalFullPreActivationBlock(
            activation, filters * 2, kernel_size=3, dilation=2, padding=2
        )
        self.preact_4 = ConditionalFullPreActivationBlock(
            activation, filters * 2, kernel_size=3, dilation=4, padding=4
        )
        ...
        self.refine_block_2 = RefineBlock(
            activation, filters * 2, n_blocks_crp=2, n_blocks_begin_rcu=2
        )
    ...
    def summary(self):
        x = [
            layers.Input(name="images", shape=self.in_shape[0][1:]),
...
class RefineNet(keras.Model):
    def __init__(
        ...
        # FIXME: In this second preactivation block we used dilation=1, but it should have been 2
        self.preact_2 = ConditionalFullPreActivationBlock(
            activation, filters * 2, kernel_size=3, pooling=True, dilation=2, padding=2
        )
...
class RefineNetLite(keras.Model):
    def __init__(self, filters, activation, y_conditioned=False, splits=None):
        super(RefineNetLite, self).__init__()
        self.in_shape = None
    ...
    def call(self, inputs, mask=None):
        ...
        # output_4 = self.refine_block_4([[output_4], idx_sigmas])
        output_3 = self.refine_block_3([[output_3], idx_sigmas])
        output_2 = self.refine_block_2([[output_2, output_3], idx_sigmas])
        output_1 = self.refine_block_1([[output_1, output_2], idx_sigmas])
...
class MaskedRefineNet(keras.Model):
    def __init__(self, filters, activation, splits, y_conditioned=False):
        ...
        self.increase_conditional_channels = layers.Conv2D(
...
class RefineNetTwoResidual(keras.Model):
    def __init__(self, filters, activation):
        ...
        self.preact_1_1 = ConditionalFullPreActivationBlock(
            activation, filters, kernel_size=3
        )
        self.preact_1_2 = ConditionalFullPreActivationBlock(
            activation, filters, kernel_size=3
        )
        # FIXME: In this second preactivation block we used dilation=1, but it should have been 2
        self.preact_2_1 = ConditionalFullPreActivationBlock(
            activation, filters * 2, kernel_size=3
        )
        self.preact_2_2 = ConditionalFullPreActivationBlock(
            activation, filters * 2, kernel_size=3, pooling=True
        )

        self.preact_3_1 = ConditionalFullPreActivationBlock(
            activation, filters * 2, kernel_size=3, dilation=2, padding=2
        )
        self.preact_3_2 = ConditionalFullPreActivationBlock(
            activation, filters * 2, kernel_size=3, dilation=2, padding=2
        )
        self.preact_4_1 = ConditionalFullPreActivationBlock(
            activation, filters * 2, kernel_size=3, dilation=4, padding=4
        )
        self.preact_4_2 = ConditionalFullPreActivationBlock(
            activation, filters * 2, kernel_size=3, dilation=4, padding=4
        )
    ...
    def build(self, input_shape):
        ...
        self.in_shape = input_shape
        self.decrease_channels = layers.Conv2D(
            input_shape[0][-1], kernel_size=3, strides=1, padding="same"
        )
