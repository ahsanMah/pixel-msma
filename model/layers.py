...
class DilatedConv2Dv2(layers.Layer):
...
class FullPreActivationBlock(layers.Layer):
    def __init__(
        self, activation, filters, kernel_size=3, dilation=1, padding=1, pooling=False
    ):
    ...
    def build(self, input_shape):
        ...
        if begin_filters != self.filters:
            self.increase_channels_skip = layers.Conv2D(
                self.filters, kernel_size=1, padding="valid"
            )
...
class ConditionalInstanceNormalizationPlusPlus2D(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.alpha = self.add_weight(
            name=self.name + "_alpha",
            shape=(self.L, 1, 1, self.C),
            initializer=self.init_weights,
        )
        self.beta = self.add_weight(
            name=self.name + "_beta",
            shape=(self.L, 1, 1, self.C),
            initializer=self.init_bias,
        )
...
class ConditionalChainedResidualPooling2D(layers.Layer):
    def __init__(self, n_blocks, activation, filters, kernel_size=3, pooling_size=5):
        ...
        for n in range(n_blocks):
            setattr(
                self, "norm1{}".format(n), ConditionalInstanceNormalizationPlusPlus2D()
            )
            setattr(
                self,
                "conv{}".format(n),
...
class MultiResolutionFusion(layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        if len(inputs[0]) == 2:
            ...
            # FIXME: Cast this to highinput dtype to ensure float16 calculations
...
class RefineBlock(layers.Layer):
    def __init__(
        ...
        self.crp = ConditionalChainedResidualPooling2D(
            n_blocks_crp, activation, filters, kernel_size, pooling_size
        )
    ...
    def build(self, input_shape):
        for n in range(self.n_blocks_begin_rcu):
            setattr(
                self,
                "rcu_high{}".format(n),
                RCUBlock(self.activation, self.filters, self.kernel_size),
            )
